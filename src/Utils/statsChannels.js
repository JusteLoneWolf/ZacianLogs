   ...
   init: (client) => {
    ...
    add: async (client,db,guild) =>{
        if(!message.guild.channels.find(c => c.name === 'Stats du serveur' && c.type === 'category')){
            var category_stats = await message.guild.channels.create('Stats du serveur', {type: 'category'});
        ...
        if(db.channels.counter.bot.id === '' || !message.guild.channels.find(c => c.id === db.channels.counter.bot && c.type === 'voice') && !message.guild.channels.find(c => c.name.startsWith('Bots : ') && c.type === 'voice')){
            channel_botcount = await message.guild.channels.create(`Bots : ${message.guild.members.filter(b => b.user.bot).size}`, {type: 'voice'}).then(channel => channel.setParent(category_stats.id)).catch((err) => {client.emit("error", err, message)});
        }else{
        ...
        if(db.channels.counter.member=== '' || !message.guild.channels.find(c => c.id === db.channels.counter.member && c.type === 'voice') && !message.guild.channels.find(c => c.name.startsWith('Utilisateurs : ') && c.type === 'voice')){
            channel_usercount = await message.guild.channels.create(`Utilisateurs : ${message.guild.members.filter(b => !b.user.bot).size}`, {type: 'voice'}).then(channel => channel.setParent(category_stats.id)).catch((err) => {client.emit("error", err, message)});
        }else{
        ...
        if(db.channels.counter.all.id=== '' || !message.guild.channels.find(c => c.id === db.channels.counter.all.id && c.type === 'voice') && !message.guild.channels.find(c => c.name.startsWith('Total : ') && c.type === 'voice')){
            channel_total = await message.guild.channels.create(`Total : ${message.guild.memberCount}`, {type: `voice`}).then(channel => channel.setParent(category_stats.id)).catch((err) => {client.emit("error", err, message)});
        }else{
        ...
        if(db.channels.counter.task.id=== '' || !message.guild.channels.find(c => c.id === db.channels.counter.task.id && c.type === 'voice') && !message.guild.channels.find(c => c.name.startsWith('Objectif : ') && c.type === 'voice')){
            channel_task = await message.guild.channels.create(`Objectif : ${message.guild.memberCount}`, {type: `voice`}).then(channel => channel.setParent(category_stats.id)).catch((err) => {client.emit("error", err, message)});
        }else{
